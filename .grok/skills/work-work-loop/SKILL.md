...
Run one phase, then stop. The kernel is `C:\dev\wwl\docs\WORK_WORK_LOOP_DRAFT_V1.txt`. Read that file before the phase. Laws, spines, and the six score axes live there. This skill is the Grok procedure around that file.

The gate is `C:\dev\wwl\engine\hybrid_gate_harness.py`. Call it only through `python -m wwl_phase` with `PYTHONPATH` and the working directory set to `C:\dev\work-work-loop`. Every command prints one JSON object. `success: false` ends the turn. Deliver `error` and stop.

## Admit

Use the run root the user names. When they do not name one, create `C:\dev\Grok Build\wwl-runs\<slug>-<yyyyMMdd>` and take the date from the shell clock.
...
From `C:\dev\work-work-loop`: `python -m unittest discover -s tests -t .`
